...
from app.api.v2.businesses.service import (
    add_member,
    create_business,
    get_business,
...
def update_member_endpoint(business_id, member_id):
    ...
    if error:
        status = 404 if error == "Member not found" else 403 if error in {
            "Only owner can manage owner membership",
            "Owner role cannot be reassigned from this endpoint",
        } else 400
        return jsonify({"error": error}), status
    return jsonify(result), 200
...
def remove_member_endpoint(business_id, member_id):
    ...
    if error:
        status = 404 if error == "Member not found" else 403 if error in {
            "Only owner can remove owner membership",
            "Cannot remove active business owner membership",
        } else 400
        return jsonify({"error": error}), status
    return jsonify(result), 200
