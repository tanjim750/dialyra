...
from app.models import Business, BusinessAccessToken, WorkspaceMembership
...
def _resolve_target_business(actor_user, business_id):
    ...
    if actor_user.role == "stuff":
        ...
        target_id = int(business_id) if business_id else actor_business_id
        if not target_id:
            return None, "Stuff account has no business"
        business = Business.query.get(target_id)
        if business is None:
            return None, "Business not found"
        membership = WorkspaceMembership.query.filter_by(
            user_id=actor_user.id, business_id=business.id, status="active"
        ).first()
        if membership is None and actor_business_id != business.id:
            return None, "Cross-business access denied"
        return business, None

    if actor_user.role == "general":
        if not business_id:
            return None, "business_id is required for general users"
        business = Business.query.get(int(business_id))
        if business is None:
            return None, "Business not found"
        membership = WorkspaceMembership.query.filter_by(
    ...
    return None, "Only superuser, stuff, or general member can manage access tokens"
...
def list_access_tokens(actor_user, business_id=None):
    ...
    if actor_user.role == "superuser":
        ...
        actor_business_id = get_primary_business_id(actor_user.id)
        if not actor_business_id and not business_id:
            return None, "Stuff account has no business"
        target_id = int(business_id) if business_id else actor_business_id
        membership = WorkspaceMembership.query.filter_by(
            user_id=actor_user.id, business_id=target_id, status="active"
        ).first()
        if membership is None and actor_business_id != target_id:
            return None, "Cross-business access denied"
        query = query.filter_by(business_id=target_id)
    elif actor_user.role == "general":
        if not business_id:
            return None, "business_id is required for general users"
        membership = WorkspaceMembership.query.filter_by(
            user_id=actor_user.id, business_id=int(business_id), status="active"
        ).first()
        if membership is None:
            return None, "Cross-business access denied"
        query = query.filter_by(business_id=int(business_id))
    else:
        return None, "Only superuser, stuff, or general member can view access tokens"
...
def get_access_token(actor_user, token_id):
    ...
    if actor_user.role == "general":
        membership = WorkspaceMembership.query.filter_by(
            user_id=actor_user.id, business_id=token_model.business_id, status="active"
        ).first()
        if membership is None:
            return None, "Cross-business access denied"
    if actor_user.role not in {"superuser", "stuff", "general"}:
        return None, "Only superuser, stuff, or general member can view access tokens"
...
def revoke_access_token(actor_user, token_id):
    ...
    if actor_user.role == "general":
        ...
        if membership is None:
            return None, "Cross-business access denied"
    if actor_user.role not in {"superuser", "stuff", "general"}:
        return None, "Only superuser, stuff, or general member can revoke access tokens"
...
def delete_access_token(actor_user, token_id):
    ...
    if actor_user.role == "general":
        ...
        if membership is None:
            return None, "Cross-business access denied"
    if actor_user.role not in {"superuser", "stuff", "general"}:
        return None, "Only superuser, stuff, or general member can delete access tokens"
