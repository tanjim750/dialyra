...
def create_business(actor_user, payload):
    ...
    if not name or not slug or not owner_name or not email:
...
def update_business(actor_user, business, payload):
    ...
    if "owner_name" in payload:
        business.owner_name = (payload.get("owner_name") or "").strip() or business.owner_name
    if "email" in payload:
...
def update_member(actor_user, business, member_id, payload):
    ...
    if business.owner_user_id and membership.user_id == business.owner_user_id:
        if "role" in payload and (payload.get("role") or "").strip().lower() != membership.role:
            return None, "Owner role cannot be changed here"

    if "role" in payload:
        role = (payload.get("role") or "").strip().lower()
        if role not in VALID_MEMBER_ROLES:
            return None, "Invalid member role"
        if business.owner_user_id and membership.user_id == business.owner_user_id and role != "owner":
            return None, "Owner role cannot be changed here"
    ...
    if "status" in payload:
        ...
        if status not in VALID_MEMBER_STATUSES:
            return None, "Invalid member status"
        if business.owner_user_id and membership.user_id == business.owner_user_id and status != "active":
            return None, "Owner status cannot be changed here"
...
def remove_member(actor_user, business, member_id):
    ...
    if business.owner_user_id and membership.user_id == business.owner_user_id:
        return None, "Owner cannot be removed from membership"
